...
def html_to_pdf(html_buffer):
    # PDFに変換するためのバッファ
    pdf_buffer = BytesIO()

    # PDFの生成
    pdfkit.from_file(html_buffer, pdf_buffer)
...
if uploaded_file:
    if st.button('Convert to PDF'):
        html_buffer = excel_to_html(uploaded_file)
        pdf_buffer = html_to_pdf(html_buffer)
        st.download_button(label="Download PDF", data=pdf_buffer, file_name="output.pdf", mime="application/pdf")
